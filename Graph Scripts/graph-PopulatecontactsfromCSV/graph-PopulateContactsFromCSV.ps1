...
"@
    ...
    do {
        ...
        Try {
                ...
                Write-Error "Error creating contact $($contact.emailaddress) for $mailbox $($_.Exception.Message)" -ErrorAction Continue
        }
    } while ($cnt -lt 5)
    throw "Unable to creat contact $($contact.EmailAddress) for $mailbox $($_.Exception.Message)"
}
